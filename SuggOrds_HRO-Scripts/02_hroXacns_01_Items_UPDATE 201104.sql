...
-- 01/05/22: Added a delete transaction to remove any items that now appear in ngXacns.
...
where pm.ItemCode not in (select ItemCode from #excludes)
	...
	and (pm.PurchaseFromVendorID in ('IDAURORA ','IDBENDONPU','IDBKSALESI','IDBOOKDEPO','IDBRYBELLY','IDC&DVISIO','IDCRAZART','IDCROWNB&C'
                                    ...
                                    ,'IDUSPLAYIN','IDWISHPETS','IFDISCONFO','IFPROPERRE','IDMAKEITRE','IDGIANTMIC','IFFATCAT','IDUSMEDIAP')
...
----Transaction 3--------------------------------
--Removing any items now in ngXacns_Items -------[
-----------------------------------------------[
BEGIN TRY
	begin transaction
	
	delete hro 
	from ReportsView.dbo.hroXacns_Items hro 
	where hro.ItemCode in (select distinct ItemCode from ReportsView..ngXacns_Items);
